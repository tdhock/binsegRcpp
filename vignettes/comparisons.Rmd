...
penaltyLearning::modelSelection(
  bs.fit$splits[, .(loss,segments)], "loss", "segments")
```
...
for(penalty in seq(0, 50)){
  ...
  pen.changepoint.list[[paste(penalty)]] <- data.table(
    package="changepoint",
...
```{r}
bs.fit$splits[, changepoint.loss := loss*2]
(pen.df <- penaltyLearning::modelSelection(
  bs.fit$splits[, .(
    package="binsegRcpp+penaltyLearning",
    changepoint.loss, segments
  )], "changepoint.loss", "segments"))
gg.penalty+
  geom_segment(aes(
    min.lambda, segments,
